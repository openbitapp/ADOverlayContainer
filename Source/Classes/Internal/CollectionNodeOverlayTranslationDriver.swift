...
class CollectionNodeOverlayTranslationDriver: OverlayTranslationDriver, OverlayScrollViewDelegate {

    weak var translationController: OverlayTranslationController?
    weak var collectionNode: ASCollectionNode?

    private let collectionNodeDelegateProxy = OverlayCollectionNodeDelegateProxy()
    ...
    init(translationController: OverlayTranslationController, collectionNode: ASCollectionNode) {
        self.translationController = translationController
        self.collectionNode = collectionNode
        collectionNodeDelegateProxy.forward(to: self, delegateInvocationsFrom: collectionNode)
        lastContentOffsetWhileScrolling = collectionNode.contentOffset
    }

    // MARK: - OverlayTranslationDriver

    func clean() {
        collectionNodeDelegateProxy.cancelForwarding()
    }
