...
class TableNodeOverlayTranslationDriver: OverlayTranslationDriver, OverlayScrollViewDelegate {

    weak var translationController: OverlayTranslationController?
    weak var tableNode: ASTableNode?

    private let tableNodeDelegateProxy = OverlayTableNodeDelegateProxy()
    ...
    init(translationController: OverlayTranslationController, tableNode: ASTableNode) {
        self.translationController = translationController
        self.tableNode = tableNode
        tableNodeDelegateProxy.forward(to: self, delegateInvocationsFrom: tableNode)
        lastContentOffsetWhileScrolling = tableNode.contentOffset
    }

    // MARK: - OverlayTranslationDriver

    func clean() {
        tableNodeDelegateProxy.cancelForwarding()
    }
